WITH 
  ...
  ----------------------------------------------------------------------------
  -- 3) Rename final supply column to match your usage
  ----------------------------------------------------------------------------
  crv_supply AS (
    SELECT
      time AS date,             -- same date name used in next join
      cumm AS circulating_supply
    FROM crv_mint
  ),

  ----------------------------------------------------------------------------
  -- 4) Hourly price data (unchanged)
  ----------------------------------------------------------------------------
  price_data AS (
    SELECT 
      HOUR,
      PRICE,
      PRICE * (2220059525285733442262148468 / 1e18) AS MARKET_CAP
    FROM ethereum.price.ez_prices_hourly
    WHERE CAST(HOUR AS TIME) = '00:00:00'
      AND TOKEN_ADDRESS = lower('0xd533a949740bb3306d119cc777fa900ba034cd52')
  ),

  ----------------------------------------------------------------------------
  -- 5) Match supply and price data 
  ----------------------------------------------------------------------------
  matched_data AS (
    SELECT 
      p.HOUR,
