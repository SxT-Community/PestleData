...
combined_daily AS (
  SELECT
    ...
    COALESCE(daily_airdrop, 0) AS daily_airdrop,
    COALESCE(daily_rewards, 0) AS daily_rewards,
    COALESCE(daily_airdrop, 0) + COALESCE(daily_rewards, 0) AS new_daily_circulating
  FROM airdrop_claims ac
  FULL OUTER JOIN rewards_claimed rc
    ON ac.day = rc.day
),

daily_circulating_cumulative AS (
  SELECT
    day,
    daily_airdrop,
    daily_rewards,
...
SELECT *
FROM daily_circulating_cumulative
ORDER BY day desc;

-- Source: https://docs.eigenfoundation.org/eigen-token/key-info